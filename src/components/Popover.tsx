...
export default function Popover(props: ParentProps<{ contents: (props: { close: () => void }) => JSX.Element, noPadding?: boolean }>): JSX.Element {
    ...
    return <div class="popover-wrapper">
        ...
        <div style={`left:${(rect()?.x ?? 0) + (rect()?.width ?? 0) / 2}px;top:${(rect()?.bottom ?? 0) + 4}px`} class={`popover overflow-hidden z-[999] shadow-black/0.4 shadow-lg fixed -translate-x-1/2 transition-all border-muted ${props.noPadding ? "": "p-4 "}rounded-lg bg-bg ${open() ? "opacity-100 scale-100" : "scale-90 opacity-0 pointer-events-none"}`}>
            <props.contents close={() => setOpen(false)} />
        </div>
        {open() && <div class="fixed top-0 left-0 w-screen h-screen z-[998]" onClick={() => setOpen(false)} /> }
    </div>
}