...
export default function Nav(props: { blank?: boolean }): JSX.Element {
    return <div data-tauri-drag-region class="navbar h-14 bg-bg select-none flex justify-between items-center px-6 fixed top-0 inset-x-0 z-[999]">
        {!open()
            ? <A href={props.blank ? useLocation().pathname : "/"} class="echoir-icon">
            ...
            : <A href="#" class="back-button w-[78px] cursor-default">
                <svg stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" class="h-6 cursor-pointer" onClick={() => setOpen(false)}>
                    <path stroke-width={3} d="M15.75 19.5 8.25 12l7.5-7.5"></path>
                </svg>
            </A>
        }
