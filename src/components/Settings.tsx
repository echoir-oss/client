...
export const SettingsProvider = (props: ParentProps) => (
    <SettingsContext.Provider value={{ value: [open, setOpen] }}>
        {props.children}
...
const Pages = {
    ...
    [SettingsPage.Themes]: () => (
        <div>
            <h1 class="font-bold text-3xl">Themes</h1>
            <h3 class="mb-2 text-xl">
                Manage your themes and custom styles for Echoir.
            </h3>
            <div class="px-2 py-1.5 border border-white/25 rounded-lg w-fit">
                <h3 class="mb-1 font-bold text-muted text-sm">Actions</h3>
    ...
    [SettingsPage.About]: () => {
        ...
        const messages = [
            "It's time to ditch Discord and go with Echoir.",
            "Made by developers, tailored for you.",
        ...
        return (
            <div class="flex flex-col justify-center items-center gap-2 w-full h-full">
                ...
                <h3 class="relative w-full h-7 overflow-hidden text-xl">
                    {state() !== TransitionState.None && (
                        <span
                            class={`transition-transform duration-1000 w-full flex justify-center absolute left-0 top-0 ${state() === TransitionState.BeforeRun ? "translate-y-0" : "-translate-y-full"}`}
                        >
                            {messages[last()]}
                        </span>
                    )}
                    <span
                        class={`transition-transform duration-1000 w-full flex justify-center absolute left-0 top-0 ${state() === TransitionState.BeforeRun ? "translate-y-full invisible" : "translate-y-0"}`}
                    >
                        {messages[curr()]}
                    </span>
                </h3>
                <div class="mt-2">
                    crafted with &lt;3 by{" "}
                    <a
                        href="https://github.com/WorriedArrow"
                        target="_blank"
                    ...
                    ,{" "}
                    <a
                        href="https://github.com/CurrentlyEmilia"
                        target="_blank"
                        rel="noopener noreferrer"
                        class="text-brand underline"
                    >
                    ...
                    , and{" "}
                    <a
                        href="https://github.com/echoir-oss"
                        target="_blank"
                        rel="noopener noreferrer"
                        class="text-brand underline"
                    >
                ...
                <div class="mt-[-5px] text-muted text-sm">
                    Echoir is{" "}
                    <a
                        href="https://github.com/echoir-oss"
                        target="_blank"
                        rel="noopener noreferrer"
                        class="text-brandDark underline"
                    >
                        open source
                    </a>{" "}
                    and built on{" "}
                    <span
                        onClick={() => setPage(SettingsPage.Licenses)}
    ...
    [SettingsPage.Licenses]: () => (
        <div>
            <h1 class="font-bold text-3xl">Licenses</h1>
            <h3 class="text-xl">
                Review the licenses of software and libraries used by Echoir.
            </h3>
            <div class="flex flex-col">
                <div>
                    <span class="font-bold">Echoir</span> /{" "}
                    <span class="text-white/70">MIT license</span>
                </div>
                <div>
                    <a
                        href="https://github.com/nksaraf/vinxi"
                        target="_blank"
                        rel="noopener noreferrer"
                ...
                <div>
                    ...
                    / <span class="text-white/70">MIT/Apache 2.0 license</span>
                </div>
            </div>
        </div>
    ),
    ...
    [SettingsPage.Routes]: () => (
        <div>
            <h1 class="font-bold text-3xl">Routes</h1>
            <h3 class="text-xl">Below is a list of a bunch of routes.</h3>
            <div class="flex flex-col gap-2">
                <A href="/" class="text-brand underline">
                    Home
                </A>
                <A href="/notfounderror" class="text-brand underline">
                    Not Found Error
                </A>
                <A href="/error" class="text-brand underline">
                    UI Error / Component Crash
                </A>
            </div>
        </div>
    ),
    [SettingsPage.Components]: () => (
        <div>
            ...
            <h3 class="text-xl">
                Below is a list of common UI components (
            ...
            <div class="flex flex-col gap-2 *:w-max">
                ...
                <ButtonBack onClick={() => console.log("Button clicked!")}>
                    {"<ButtonBack />"}
                </ButtonBack>
            </div>
        </div>
    ...
    [SettingsPage.Plugins]: () => (
        <div class="h-[calc(100%-3.5rem)]">
            <h1 class="font-bold text-3xl">Plugins</h1>
            <h3 class="mb-2 text-xl">
                Customize your client’s functionality with officially verified
                plugins!
            </h3>
            <TabBar>
                <Tab name="Official">
                    <div class="gap-3.5 grid grid-cols-3">
                        <div class="flex flex-col justify-between bg-layer drop-shadow-lg p-3 rounded-md min-h-40 max-h-40">
                            <div class="flex flex-col gap-1 px-2">
                                <div class="flex items-end gap-1.5">
                                    <div class="font-semibold text-2xl">
                                        DiscordBridge
                                    </div>
                                    <div class="text-[15px] text-white/65 break-all text-ellipsis line-clamp-1 -translate-y-[3px]">
                                        by patricktbp
                                    </div>
                                </div>
                                <div class="text-ellipsis line-clamp-2">
                                    Add a bridge from Discord to the Echoir
                                    client with 100% functionality!
                                </div>
                            </div>
                            <div class="flex justify-end gap-1">
                                <button
                                    type="button"
                                    class="bg-buttonBack px-4 py-1 border border-[#444] rounded-l rounded-r-sm text-[15px]"
                                >
                                    Details
                                </button>
                                <button
                                    type="button"
                                    class="bg-buttonBack px-4 py-1 border border-[#444] rounded-r rounded-l-sm text-[15px]"
                                >
                                    Install
                                </button>
                            </div>
                ...
                <Tab name="Third-Party">
                    <div class="flex justify-center items-center w-full h-full">
                        <div class="drop-shadow-lg">
                            <div class="flex flex-col justify-center items-center bg-layer px-8 py-4 rounded-md w-[500px] text-center warning-box">
                                <svg
                                    width="50"
                                    height="50"
                                    viewBox="0 0 50 50"
                                ...
                                <p class="mb-1">
                                    Third-party plugins haven't been verified by
                                    the Echoir team and could be risky. If
                                    you're unsure, it's best not to continue. If
                                    you're confident in what you're doing,
                                    proceed with a bit of caution. If you're a
                                    developer, you can test your plugins here to
                                    make sure everything works as intended.
                                </p>
                                <A
                                    href="https://example.com"
                                    class="mb-2 text-linkColor"
                                >
                                    More information can be found here
                                </A>{" "}
                                {/* Please replace this with an actual URL at some point! */}
                                <div class="flex flex-row gap-1 button-box-warning">
                                    <ButtonBack
                                        onClick={() =>
...
export default function Settings(): JSX.Element {
    ...
    const Section = (props: ParentProps) => (
        <div class="mb-1 px-2 font-semibold text-muted text-sm settings-section-title">
            {props.children}
        </div>
    );
    const Separator = () => (
        <div class="m-2 border-t border-t-white/10 settings-separator" />
    );
    ...
    return (
        <div
            class={`settings fixed flex gap-4 w-screen justify-center top-14 bg-bg h-[calc(100vh-3.5rem)] px-3 pb-3 z-30 transition-[opacity,transform] ease-out duration-200 ${open() ? "opacity-100 scale-100" : "opacity-0 scale-150 pointer-events-none"}`}
        >
            <aside class="flex flex-col bg-bgLayer p-4 rounded-xl min-w-64 h-full settings-nav">
                <Settings.Section>User Settings</Settings.Section>
                <Settings.Item page={SettingsPage.Account}>
                    Account
                </Settings.Item>
                <Settings.Separator />
                <Settings.Section>Customization</Settings.Section>
                <Settings.Item page={SettingsPage.Themes}>Themes</Settings.Item>
                <Settings.Item page={SettingsPage.Messages}>
                    Messages
                </Settings.Item>
                <Settings.Item page={SettingsPage.Plugins}>
                    Plugins
                </Settings.Item>
                <Settings.Separator />
                <Settings.Section>Miscellaneous</Settings.Section>
                <Settings.Item page={SettingsPage.About}>
                    About Echoir
                </Settings.Item>
                <Settings.Item page={SettingsPage.Licenses}>
                    Licenses
                </Settings.Item>
                <Settings.Separator />
                {DEV && (
                    <>
                        ...
                        <Settings.Item page={SettingsPage.Routes}>
                            Routes
                        </Settings.Item>
                        <Settings.Item page={SettingsPage.Components}>
                            Components
                        </Settings.Item>
                        <Settings.Separator />
                    </>
                )}
                <div class="mb-1 px-2 text-white/50 text-xs settings-version-info">
                    <p>
                        Echoir {tauri.appVersion}
                        {DEV && "@dev"} ({COMMIT_HASH})
                    </p>
            ...
            <div class="bg-bgLayer p-4 rounded-xl w-full h-[calc(100vh-3.5rem)] overflow-x-hidden overflow-y-auto settings-page">
                {Pages[page()]()}
            </div>
        </div>
    );
