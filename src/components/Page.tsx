...
export default function Page(
    ...
    return (
        ...
        >
            ...
            <Show
                when={!props.unauthed && (me.error || (me() as any)?.message)}
            >
                <Navigate href="/auth/signin" />
            </Show>
            <Show when={props.unauthed || me() !== undefined}>
                <main
                    class={`pt-14 pl-4 pr-3 pb-3 h-screen flex gap-4 transition-transform ease-out duration-200 ${useContext(SettingsContext).value[0]() ? "scale-75" : "scale-100"}`}
                >
                    <div class="w-16 h-full server-bar">
                        <A
                            href="/"
                            class="relative flex justify-center items-center bg-bgLayer rounded-xl w-16 h-16 server-button home-button"
                        >
                            <svg
