...
export const createRootCache = <K extends PropertyKey, V>(
    gen: (key: string) => Promise<V>,
): RootCache<K, V> => {
    const initialized: Record<K, Promise<V>> = {} as Record<K, Promise<V>>;
    const cache: RootCache<K, V> = {
        value: new Proxy({} as Record<K, () => Promise<V>>, {
            get: (_, key) => () =>
                (initialized[key as K] ??= gen(key as string)),
        }),
        invalidate: (key, value) =>
            (initialized[key as K] = Promise.resolve(value)),
    };
    return cache;
};
